...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
	...
	// C
	a = 1.0078*g/mole;
	G4Element* H  = new G4Element(name="CHydrogen" ,symbol="H" , z= 1., a);
	...
	G4Element* Ce = new G4Element(name="Cerium",symbol="Ce" , z= 58., a);
	 // Sr
	a = 87.62*g/mole;
	G4Element* Sr = new G4Element(name="Strontium",symbol="Sr" , z= 38., a);
	//
	...
	// LaBr3
	density = 5.08*g/cm3;
	G4Material* LaBr3 = new G4Material(name="LaBr3", density, ncomponents=4);
	LaBr3->AddElement(Br, natoms=3);
	LaBr3->AddElement(La, natoms=1);
	LaBr3->AddElement(Ce, natoms=1);
	LaBr3->AddElement(Sr, natoms=1);
	...
	// opthel
	density = 1.06*g/cm3;
	G4Material* Optmat = new G4Material(name="optgelmat", density, ncomponents=1);
	Optmat->AddElement(Si, natoms=1);
...
G4Material* fOptgrease = new G4Material("Optical_grease", density= 1.12*g/cm3, 4); //highest density 1.05-1.12 g/cm3 C7H8OSi
...
fOptgrease->AddElement(Si,1);
	...
	G4double PhotonEnergy[nEntries] = {3.76	*eV, 2.49	*eV};
	...
	LaBr3MPT->AddProperty("FASTCOMPONENT",PhotonEnergy,ScintFast,nEntries);
	LaBr3MPT->AddConstProperty("SCINTILLATIONYIELD",73./keV); 
	LaBr3MPT->AddConstProperty("RESOLUTIONSCALE",1.);
	LaBr3MPT->AddConstProperty("FASTTIMECONSTANT",25.0*ns);
	...
	G4double BGO_Energy[nEntries] = {3.13*eV, 3.08};
	G4double BGO_Fast[nEntries] = {1.00	, 0.95};
	 
 BGO_MPT->AddProperty("FASTCOMPONENT",BGO_Energy,BGO_Fast,nEntries);
	BGO_MPT->AddConstProperty("SCINTILLATIONYIELD",10.0/keV); 
	BGO_MPT->AddConstProperty("RESOLUTIONSCALE",1.);
	BGO_MPT->AddConstProperty("FASTTIMECONSTANT",20.*ns);
	BGO_MPT->AddConstProperty("YIELDRATIO",1.);
...
G4double BGOW_Z = 60.1*mm;
	...
	G4VPhysicalVolume* physireflector_al = new G4PVPlacement(0,
	...
	G4VPhysicalVolume* physireflector_alface = new G4PVPlacement(0,
	                                                    G4ThreeVector(0.*cm,0.*cm,-(Reflector_Z/2)+((0.1*cm)/2)),
							   "reflectorface",
							   lreflector_alface,
							   physiWorld,
							   false,
...
//Housing
	...
	G4VPhysicalVolume* physihousing_al = new G4PVPlacement(0,
	                                                    G4ThreeVector(0.*cm,0.*cm,0.*cm),
							   "physichousing",
							   lhousing_al,
							   physiWorld,
	...
	G4VPhysicalVolume* physihousing_alface = new G4PVPlacement(0,
	                                                    G4ThreeVector(0.*cm,0.*cm,-(Reflector_Z/2)+0.1*cm+((0.05*cm)/2)),
							   "reflectorface",
							   lreflector_alface,
							   physiWorld,
							   false,
	...
	G4VPhysicalVolume* physiLaBr3  = new G4PVPlacement(0,
							G4ThreeVector(0.*cm,0.*cm,-(Reflector_Z/2)+0.05*cm+LaBr3Z/2),
							 "Physi_LaBr3",
							 lLaBr3,
							 physiWorld,
							 false,
	...
	G4VPhysicalVolume* physitef  = new G4PVPlacement(0,
							                                          G4ThreeVector(0.*cm,0.*cm,0.*cm),
							                                           "teflon",
							                                           lteflon,
							                                           physiWorld,
...
// window
	...
	G4VPhysicalVolume* physiglassWindow = new G4PVPlacement(0,
	                                                      G4ThreeVector(0.*cm,0.*cm,-(Reflector_Z/2)+0.05*cm+LaBr3Z+(Glass_Z/2)),
							     "Glass_window",
							     lglassWindow,
							     physiWorld,
							     false,
...
G4VPhysicalVolume* physiOptgel = new G4PVPlacement(0,
                                                   G4ThreeVector(0.*cm,0.*cm, -(Reflector_Z/2)+0.05*cm+LaBr3Z+(Glass_Z)+(LaBr3Z/1000)),
						                                             "optgel",
						                                             lOptgel,
						                                             physiWorld,
						                                             false,
...
physiBGOW = new G4PVPlacement(rotationMatrix,
                                         G4ThreeVector(std::sin((j)*12.857*deg)*31.25*mm,std::cos(>
                                                              "BGOW",
                                                              lBGOW,
                                                              physiWorld,
                                                              false,
...
G4VPhysicalVolume* physiBGO = new G4PVPlacement(0,
                                         G4ThreeVector(std::sin((0)*12.857*deg)*0.*mm,std::cos((0)>
                                                              "BGO",
                                                              lBGO,
                                                              physiBGOW,
                                                              false,
...
*/
 ...
 for(G4int i=0; i<52; i++){
                       	...
                       	new G4PVPlacement(0,
					G4ThreeVector(SP_X[i],SP_Y[i],  -(Reflector_Z/2)+0.05*cm+LaBr3Z+(Glass_Z)+(2*LaBr3Z/1000)+(SiPM_Z/2)),
				       "Physi_SiPM",
				       lSipm,
				       physiWorld,
				       true,
...
rotationMatrix->rotateZ((j)*12.857*deg);
physiBGOW = new G4PVPlacement(rotationMatrix,
                                         G4ThreeVector(std::sin((j)*12.857*deg)*31.25*mm,std::cos((j)*12.857*deg)*31.25*mm,0.*cm),
                                                              "BGOW",
                                                              lBGOW,
                                                              physiWorld,
                                                              false,
...
G4VPhysicalVolume* physiBGO = new G4PVPlacement(0,
                                         G4ThreeVector(std::sin((0)*12.857*deg)*0.*mm,std::cos((0)*12.857*deg)*0.*mm,0.*cm),
                                                              "BGO",
                                                              lBGO,
                                                              physiBGOW,
                                                              false,
