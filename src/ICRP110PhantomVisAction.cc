...
{
  G4cout << "________________________________________________________________ICRP110PhantomVisAction::Draw() called." << G4endl;
  // Get actual vis manager
  G4VVisManager* pVisManager = G4VVisManager::GetConcreteInstance();
  if (!pVisManager) return;

  auto container = fpDetectorConstruction->GetPhantomContainer();
  G4cout << "nVOx" << container->GetLogicalVolume()->GetNoDaughters() << G4endl;
